...
class MOERouter(torch.nn.Module):
    def __init__(self, 
        ...
        self.router_type = gate_type.get('type', 'top')
        self.is_gshard_loss = is_gshard_loss

        # gate settings
        assert self.router_type in ['top', 'cosine_top', 'experts_choice','adaptive']
        
        try:
            single_gate = importlib.import_module(f'....gates.{self.router_type}' , __name__)
        except ModuleNotFoundError:
            raise Exception("Unrecognized gate_type: %s" % self.router_type)
    ...
    def forward(self, 
        ...
        if self.router_type == 'cosine_top' or self.router_type == 'top':
    ...
    def topk_routing(self,loss_fn,gate_logits):
        scores = F.softmax(gate_logits, dim=-1) # [B, L, D]
